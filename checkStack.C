...
void checkStack(const char* name = "o2sim.root")
{
  ...
  for (int i = 0; i < mcbr->GetEntries(); ++i) {
	...
	if (t.getMotherTrackId() != - 1 && (t.GetStartVertexCoordinatesX() <= 20  &&  t.GetStartVertexCoordinatesY() <= 20  && (t.GetStartVertexCoordinatesZ() < 350 && t.GetStartVertexCoordinatesZ() > 320)))
	{
		/* 
 		*
 		*
 		*/
		std::cout << "Mother Track ID: " << t.getMotherTrackId() << "\n";				
		std::cout << "Mass" << t.GetMass() << "\n";				
		std::cout << "Energy E  = " << t.GetEnergy()<<"\n"; 		
		std::cout << "Transverse Momentum Pt = " << t.GetPt()<<"\n"; 
		std::cout <<"Start Vertex Momentum p = {"<<t.Px()<<", "<< t.Py()<<", "<< t.Pz() << "}\n";
		std::cout << "Start Vertex Coordinates v = {"<<t.Vx()<<", "<< t.Vy()<<", "<< t.Vz() << "}\n\n\n";
	}
